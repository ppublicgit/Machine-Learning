...
def dtreeID3(data, targets, features, **kwargs):
    ...
    def make_tree(data, classes, featureNames, maxlevel=-1, level=0, forest=0):
        ...
        if nData == 0 or nFeatures == 0 or (maxlevel >= 0 and level > maxlevel):
            ...
            gain, cutoffPoints = np.zeros(nFeatures), np.zeros(nFeatures)
            ...
            for featureIndex in featureSet:
                ...
                g = calc_info_gain(data, classes, featureIndex, cutoffPoint)
            ...
            for value in values:
                ...
                for index, datapoint in enumerate(data):
                    if matches(datapoint[bestFeature], value):
    ...
    def get_cutoff_point(data, classes, feature):
        ...
        for idx, item in enumerate(sort_zipped):
            ...
            prev = item
        cutoffGains, cutoffPoints = np.zeros(len(boundaries)), np.zeros(len(boundaries))
        for cIdx, boundary in enumerate(boundaries):
            testCutoff = sort_zipped[boundary][0]
            cutoffGains[cIdx] = calc_info_gain(data, classes, feature, testCutoff)
...
def dtreeCART(data, targets, features, **kwargs):
    def classify(tree, datapoint):
        if tree is None:
            return None
        elif tree.isLeaf():
            ...
            for idx, feature in enumerate(featureNames):
                if feature == tree.key:
                    break
            subtree = tree.subtree(datapoint[idx])
            if subtree is None and outtype == "regression":
                breakpoint()
            return classify(subtree, datapoint)
    ...
    def make_tree(data, classes, featureNames, maxlevel=-1, level=0, forest=0):
        ...
        if outtype == "classification" and (nData == 0 or nFeatures == 0 or (maxlevel >= 0 and level > maxlevel)):
            ...
            return DecisionTree(classes[0], [])
        #elif outtype == "regression" and nData < 3:
